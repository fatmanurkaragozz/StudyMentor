...
📅 18. Gün: 12 Ağustos 2026 (Çarşamba)

Bugün ikiye bölünmüş bir gündü: sabah tarafı tamamen "mimari düşünmek", öğleden sonrası ise tamamen "görsel/estetik düşünmek" oldu. İkisi de bana aynı şeyi öğretti aslında - bir şeyin "doğru görünmesi" ile "gerçekten doğru olması" arasındaki farkı kontrol etmeden asla emin olamıyorsun.

- **Modül Sınırları Taslağı:** Kod tabanını gerçekten tarayıp (hangi servis hangi Prisma tablosuna dokunuyor) bir mimari taslak çıkardım (`Documents/Mimari_Modul_Sinirlari_Taslagi.md`) - Identity, Curriculum, ExamTracking, LearningEngine, AICoach, Growth, ml-service diye 7 modül tanımladım. Her birinin altına bugün gerçekten ihlal edilen sınırları (kod referanslarıyla) not düştüm, 6 karar noktasını kendi önerimle birlikte yazıp onay için sundum.
- **AICoach'ın Yazma Tarafının Ayrılması:** `studySessions.service.ts` ve `topicChecks.service.ts`'in kendi işlerini bitirince doğrudan `AIRecommendation` tablosuna yazması sorununu çözdüm - yeni bir `scoreAndRecommend()` fonksiyonu (AICoach/`recommendations.service.ts`) bu işi üstlendi, LearningEngine artık sadece bunu çağırıyor. Aynı anda `Topic.lastStudied`/`nextReview` güncellemesini de (aynı kod bloğunun içindeydi) `markTopicReviewed()` altında Curriculum'a topladım.
- **Kalan Çapraz Okumaların Temizlenmesi:** İlk taramamın eksik olduğunu fark ettim - `studySessions.service.ts` ve `topicChecks.service.ts` de doğrudan `Topic` tablosuna erişiyormuş. Bunları `topics.service.ts`'e eklediğim `getTopicWithSubject`/`requireTopicInSubject` fonksiyonlarına yönlendirdim. En ilginç bulgu: `exams.service.ts`'teki `getExamCatalog` fonksiyonunun hiç `Exam` tablosuna dokunmadığını fark ettim - tamamen Curriculum'un işiymiş, yanlış dosyada duruyormuş. `subjects.service.ts`'e taşıdım.
- **Redux Sohbeti:** Neden Redux kullanmadığımızı sordum kendime aslında (ya da bana soruldu) - Context API'nin bizim ölçeğimizde neden yeterli olduğunu, Redux'ın ne zaman değerli olacağını konuştuk. Asıl fark ettiğim şey: bizim gerçek eksiğimiz Redux değil, "server state" yönetimiydi - Dashboard'daki Kaptan banner'ı ile AIInsights sekmesinin aynı öneri verisini iki ayrı, birbirinden habersiz kaynaktan çektiğini (16. günden beri bilinen bir sorun) örnek gösterdim.
- **Okyanus Shader Arka Planı:** Gerçek bir deniz fotoğrafı referans alıp mevcut GLSL shader'ı (`OceanShaderBackground.tsx`) geliştirdim: dikey renk gradyanı, köpük katmanı, yönlü dalga dokusu eklendi, hareket "hafif" olsun diye yavaşlatıldı. Yolda gerçek bir hata buldum: dikey gradyan ilk denemede neredeyse hiç görünmüyordu - sebebi, arka plan düzleminin ekrandan 3 kat büyük ölçeklenmiş olması, yani `vUv` koordinatının sadece dar bir orta dilimi ekranda görünüyordu. `gl_FragCoord` (gerçek ekran pikseli) kullanarak düzelttim.
- **Playwright'ı İlk Kez Görsel İterasyon İçin Kullandım:** Daha önce Playwright'ı sadece hata bulmak için kullanmıştım; bugün ilk kez bir görseli "beğenene kadar" ekran görüntüsü alıp gözle karşılaştırarak ayarladım (renk, köpük eşiği, hareket hızı) - kod yaz, ekran görüntüsü al, bak, ayarla, tekrarla. Bir ressamın paleti karıştırırken gözle ayar yapması gibi bir şeymiş meğer.
- **Tasarım Bütünlüğü:** Açık ve koyu mod ekran görüntülerimi paylaştığımda, logo/buton/kartların (amber+rose+emerald karışımı) okyanus temasıyla çattığını ve hero alanının altında arka planın aniden düz bir renge dönüştüğünü ("dikiş" hissi) fark ettik. "Lacivert + Altın" nautik yönünde karar kılıp tek bir renk sistemine geçtim: logo, CTA butonu, filigran yazılar, "VS" rozeti tek bir altın tonuna, "Gelişim" kartı emerald yerine okyanus paletine uygun cyan'a çevrildi; hero altındaki geçiş rengi gerçek gradyan durağıyla eşleştirilip "dikiş" hissi azaltıldı; shader'ın köpük kenarları keskinleştirilip ince bir parıltı katmanı eklendi.

Bugün en çok şunu öğrendim: hem kodda hem tasarımda "bütünlük" dediğimiz şey aslında bir sürü küçük, birbirinden bağımsız görünen kararın (hangi tablo kime ait, hangi renk hangi renkle gidiyor) toplamıymış. Tek tek bakınca hiçbiri büyük bir sorun değil ama hepsi birikince "bir şeyler yanlış ama ne olduğunu tam söyleyemiyorum" hissi yaratıyor - hem mimaride hem renkte. Ve ikisinde de çözüm aynıydı: adım adım, gerçekten bakarak (kodda grep ile, tasarımda ekran görüntüsüyle) kontrol etmek.

---

📅 19. Gün: 13 Ağustos 2026 (Perşembe)

**O gün neler yaptım:**

- **Aralıklı Tekrar Hatırlatma Sistemi (TopicReminder) — Uçtan Uca:** Kullanıcının bir konuyu bitirdiğinde ML'in "bu gidişata göre N günde bir bu konuyu tekrar etmen önemli" önerisini somut bir hatırlatmaya dönüştürme fikrini uyguladım. Yeni bir `TopicReminder` Prisma modeli (kullanıcı-konu başına tek kayıt, `@@unique([userId, topicId])`, aralık gün sayısı, bir sonraki hatırlatma tarihi, aktif/pasif durumu) ve migration eklendi. `topicReminders.service.ts` içinde `proposeReminder` (ML önerisinden hatırlatma teklifi üretme), `respondToReminder` (kullanıcı Evet/Hayır derse kaydetme; aralığı kullanıcı isterse kendi değiştirebiliyor), `listDueReminders` (bugün/geçmiş tarihli aktif hatırlatmaları listeleme) ve `advanceReminderIfActive` (kullanıcı konuyu tekrar çalışınca bir sonraki tarihi otomatik ileri alma) fonksiyonları yazıldı; hem `studySessions.service.ts` hem `topicChecks.service.ts` akışlarına entegre edildi.
- **Hatırlatma Arayüzü:** Ortak bir `ReminderPrompt.tsx` bileşeni (aralık gün sayısını +/- ile ayarlayabilme, Evet/Hayır) yazılıp hem `StudyPlanner`'ın oturum-sonu ekranına hem `TopicCheckModal`'ın sonuç ekranına eklendi. Dashboard'daki "Bugün Tekrar Zamanı" kartı, hem ML kaynaklı hem hatırlatma kaynaklı konuları `source: 'ml' | 'reminder'` alanıyla birleştirip tek listede gösterecek, hatırlatma kaynaklı olanları da küçük bir zil rozetiyle ayırt edilebilecek şekilde genişletildi.
- **"Uğraşlarım" Paneli Düzeltmesi (Gelişim Modu):** StudyPlanner'ın odak/sayaç kısmına, Gelişim modundaki kullanıcıların önceden eklediği uğraşları seçip doğrudan sayacı başlatabileceği bir yan panel eklendi. İlk aklıma gelen `getTopics()` çağrısını kullanmadan önce seed dosyasını taradım ve LIFELONG_LEARNER için globalce seed'lenmiş örnek derslerin ("Yazılım Geliştirme", "Kişisel Gelişim") bu listeye karışacağını fark ettim - kullanıcının ders kısmıyla uğraş kısmını karıştırmama isteğine ters düşerdi. Bunun yerine sadece kullanıcıya özel dersleri döndüren `getMySubjects()` kullanıldı.
...
📅 24. Gün: 20 Ağustos 2026 (Perşembe)

**O gün neler yaptım:**

- **Öğrenci/Gelişim Modlarının Tam Ayrımı:** `EducationLevel`'e, okul veya üniversiteye hiç bağlı olmadan bağımsız bir sınava (AGS, KPSS, ALES, DGS, YÖKDİL, YDS) hazırlanan kullanıcılar için yeni bir `EXAM_PREP` değeri eklendi; bu kullanıcılar için `/topics`'teki küresel katalog artık eğitim seviyesi yerine seçtikleri sınav kategorisine göre geliyor. Daha önemlisi, gerçek bir veri sızıntısı tespit edilip düzeltildi: kullanıcıya özel ders/uğraşlar hiçbir zaman hangi modda (Öğrenci/Gelişim) eklendiğini işaretlemiyordu, bu yüzden bir modda eklenen bir ders diğer modun listesinde de görünebiliyordu. Yeni bir `UserMode` enum'u ve `Subject.mode` alanıyla bu ayrım artık veritabanı seviyesinde garanti altına alındı; onboarding'e de "Bağımsız Sınav Hazırlığı" seçeneği ve landing page'e bu ayrımı netleştiren metin eklendi.
- **Derslerim/Uğraşlarım → Tablo + Sayfalama:** Ders/uğraş sayısı arttıkça sayfa aşırı uzuyordu; kart listesi bir tabloya çevrildi, 8'er satırlık sayfalama eklendi, satır-içi yeniden adlandırma ve genişletilebilir alt-satırda konu ekleme/silme/yeniden adlandırma getirildi. Bunun için backend'de daha önce hiç olmayan ders ve konu yeniden adlandırma (`PATCH`) ile konu silme (`DELETE`) uç noktaları ilk kez yazıldı.
- **Pomodoro Zamanlayıcısının Sabitlenmesi:** StudyPlanner'da yan taraftaki ders/uğraş listesi uzadıkça zamanlayıcı kutusunun içeriğinin aşağı doğru kaydığı fark edildi — kök neden, CSS Grid'in iki sütunu varsayılan olarak eşit yükseklikte tutması (`items-stretch`) ve zamanlayıcının kendi içeriğini bu uzamış kutunun ortasına hizalamasıydı (`justify-center`). `items-start` + `sticky` ile çözüldü; yan liste de kendi içinde sayfalandı.
- **AI Analiz & Koç'un Premium Hale Getirilmesi:** Bu ekranın gerçekten yetersiz olduğuna karar verilip baştan tasarlandı: sahte, hiçbir veriye bağlı olmayan "Önerilen Kaynaklar" kutusu kaldırıldı; Dashboard'daki işlevsiz "Kontrol Et" bağlantısı (sadece Dashboard'a geri atıyordu) yerine gerçek çalışan bir mini-kontrol akışı bu ekrana taşındı; her ders/uğraş için süre istatistiği (ya da süreli oturum yoksa hangi konuların çalışıldığı) gösteren yeni bir "Çalışma Geçmişi" bölümü eklendi. Bu geçmişe, gerçek bir yapay zeka ile (önce Claude, sonra kullanıcının kendi tercihiyle Google Gemini API'sine geçilerek) kısa bir yorum ürettirilip önbelleğe alınıyor. API'ye ulaşılamadığında (kota, anahtar eksikliği) başka bir ücretli modele sessizce geçilmiyor — bunun yerine uygulamanın zaten kullandığı "Kaptan" şablon sistemi ücretsiz bir yedek olarak devreye giriyor.

Bugünün en çarpıcı anı, projede o ana kadar hiç fark etmediğim bir mimari boşluğu görmekti: "Kaptan" karakterinin bütün mesajları meğer tamamen client-side, şablon tabanlıymış — hiçbir gerçek yapay zeka çağrısı yokmuş. Bunu görünce "AI Koç" ismini gerçekten hak eden bir özellik kurmak için sıfırdan bir LLM entegrasyonu gerekiyordu, ve bunu yaparken de "kredi biterse sessizce başka bir ücretli modele geçme" gibi somut bir maliyet-kontrolü kararını da baştan tasarıma dahil etmem gerektiğini öğrendim.

---

📅 25. Gün: 21 Ağustos 2026 (Cuma)

**O gün neler yaptım:**

- **Operasyonel Bilgiyi Koda Taşımak:** ML servisini başlatırken venv aktivasyonunun execution-policy yüzünden sorun çıkarması, daha önce de (19. gün) karşılaşılmış ama sadece "hatırlanması gereken bir bilgi" olarak kaldığı için tekrar unutulmuş bir problemdi. Asıl eksik bir teknik çözüm değildi — çözüm zaten biliniyordu; eksik olan, o çözümün bir insanın hafızasında değil sistemde yaşamasıydı. `start.ps1` ve CLAUDE.md'ye eklenen kalıcı not, bu bilgiyi "birinin hatırlaması gereken bir şey"den "sistemin garanti ettiği bir şey"e taşıdı; küçük ama artık tekrar etmeyecek bir hata sınıfı kapatılmış oldu.
- **Ucuz Bir Geri Bildirim Döngüsü Tasarlamak:** AI yorumlarına beğen/beğenmeme eklenirken asıl mühendislik kararı arayüzdeki buton değil, bu geri bildirimin sisteme nasıl geri besleneceğiydi. Ayrı bir eğitim hattı (fine-tuning) ya da ek bir model kurmak yerine, geri bildirim doğrudan bir sonraki isteme bir düzeltme notu olarak enjekte edildi — yani "öğrenme" uygulama katmanında, ek maliyet ya da altyapı olmadan gerçekleşiyor. Bunun doğal bir sonucu da oldu: geri bildirim arayüzü sadece arkasında gerçek bir veri kaydı olan (yani gerçek AI'dan gelen) yorumlarda gösterildi, ücretsiz şablon yedeğinde değil — arayüz, veri modelinin garanti edemeyeceği bir şeyi kullanıcıya vaat etmemeli.

Bugünün ortak teması aslında ikisinde de aynıydı: iyi mühendislik çoğu zaman yeni bir şey inşa etmek değil, var olan bir zafiyeti — tekrar eden bir hatayı ya da pahalı olabilecek bir özelliği — sistemin kendisine, mümkün olan en az sürtünme ve en az yeni altyapıyla taşımaktır.

---

📅 26. Gün: 24 Ağustos 2026 (Pazartesi)

**O gün neler yaptım:**

- **Mod Ayrımını Gerçekten Bitirmek:** 24. günkü "Öğrenci/Gelişim ayrımı" işinin aslında yarım kaldığı ortaya çıktı. Önce bir yanlış yöne saptım — modu sunucu tarafında kullanıcının eğitim seviyesinden türetip kilitledim, ki bu aslında tasarımın kendisiyle çelişiyordu: uygulama baştan beri aynı hesapta hem öğrenci hem gelişim verisi tutabilecek gerçek bir çift-persona olarak düşünülmüştü. Bunu geri alıp doğru modele dönünce asıl iş başladı: aynı "modu unutan filtre" deseni tek tek her yerde tekrar ediyordu — küresel ders kataloğu, AI Analiz'in çalışma geçmişi, alışkanlıklar, günlük, sınavlar, son oturum kayıtları... hepsi ayrı ayrı, birbirinden habersiz şekilde aynı hatayı taşıyordu. Bu, tek bir bug'dan çok, bir özelliğin bir yerde doğru tasarlanıp diğer tüm dokunduğu yerlere tutarlı şekilde yayılmadığının işaretiydi. Her birini bulup kapattıktan sonra gerçek bir kullanıcı verisiyle iki modu da ayrı ayrı sorgulayıp sıfır sızıntı olduğunu doğruladım.
- **İleriye Dönük Veri Toplamayı Sistemleştirmek:** Projenin uzun vadeli hedeflerinden biri, ileride kendi kullanıcı verisiyle kendi modelini eğitebilmek. Bunun önündeki asıl engel yeni bir alt sistem kurmak değildi — zaten var olan bazı tablolar (mini-kontrol kayıtları gibi) kazara doğru şekle (özellikler + tahmin + gerçek sonuç) sahipti, ama AI yorumlarına verilen beğen/beğenmeme geri bildirimi her yeni üretimde üzerine yazılıp kayboluyordu. Onu üzerine-yazılan bir önbellekten, hiç silinmeyen bir geçmişe çevirdim; ayrıca öneri kartlarına da aynı geri bildirimi ekleyip, gelecekte bu veriyi dışa aktarıp modeli yeniden eğitmek için ilk (ama kasıtlı olarak henüz eğitime bağlanmamış) bir dışa aktarma script'i yazdım.
- **Basit Bir Soru, İki Büyük Mimari Boşluk:** Kullanıcı sıradan bir gözlemle geldi — "neden localhost her zaman doğrudan dashboard'a düşüyor, neden `/login` diye bir adres yok?" Cevap ararken iki ayrı, birbirini büyüten eksik ortaya çıktı: uygulamada hiç URL yönlendirmesi yoktu (her "sayfa" düz bir React state'iydi), ve oturum de tek bir uzun ömürlü token'dan ibaretti — ne yenileme, ne iptal, ne süresi-dolmuş/geçersiz ayrımı. İkisini aynı anda büyük bir yeniden yazımla değiştirmek yerine, doğrulanabilir aşamalara böldüm: önce backend'e mevcut akışı hiç bozmadan yeni bir refresh-token altyapısı (rotation, çalıntı-token tespiti, httpOnly cookie) eklendi ve bu curl ile tek başına test edildi; ancak ondan sonra frontend gerçekten bu yeni mekanizmaya geçirildi; routing'in kendisi hâlâ sırada. Bu ayrım sayesinde bir şey bozulduğunda hangi katmanın sorumlu olduğu hep netti.
